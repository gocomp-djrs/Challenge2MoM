...
class acnSolver():
    ...
    def construct_sol2_bus_new(self, acn, vm, va):

        start_time = time.time()
        self.sol2_bus = {
            acn.ourbuses[j+1].theirbus.i: [
                acn.ourbuses[j+1].theirbus.i, # i
                min(max(vm[j], acn.ourbuses[j+1].evlo), acn.ourbuses[j+1].evhi), # v
                va[j]] # theta
    ...
    def construct_sol1_new(self, acn, vm, va, clearedloads, genp, genq, genon, lineon, tron, taps, sw):
        ...
        self.sol1['loads'] = {
            ...
            #for r in self.data.sup.get_loads()
            #if self.data.raw.loads[(r['bus'], r['id'])].status == 1}
            (acn.ourloads[r].theirload.i, acn.ourloads[r].theirload.id): [
                acn.ourloads[r].theirload.i, # i
                acn.ourloads[r].theirload.id, # id
                clearedloads[acn.ourloads[r].count]] # t
            for r in range(1,1+acn.numloads)
            if acn.ourloads[r].status}
        #print('loads:',self.sol1['loads'])
        self.sol1['generators'] = {
            #(r.i, r.id): [
            #    r.i, # i
            #    r.id, # id
            ...
            #    r.stat] # x
            #for r in self.data.raw.generators.values()}
            (acn.ourgens[r].i, acn.ourgens[r].id): [
                acn.ourgens[r].i, # i
                acn.ourgens[r].id, # id
                (genp[r-1])
                if genon[r-1] else 0.0, # p
                (genq[r-1])
                if genon[r-1] else 0.0, # q
                genon[r-1]] # x
            for r in range(1, acn.numgens+1)}
        #print('gens:',self.sol1['generators'])
        self.sol1['lines'] = {
            #(r.i, r.j, r.ckt): [
            #    r.i, # iorig
            #    r.j, # idest
            ...
            #    r.st] # x
            #for r in self.data.raw.nontransformer_branches.values()}
            (acn.ournontrans[r].i, acn.ournontrans[r].j, acn.ournontrans[r].ckt): [
                acn.ournontrans[r].i, # iorig
                acn.ournontrans[r].j, # idest
                acn.ournontrans[r].ckt, # id
                lineon[r-1]] # x
            for r in range(1,acn.nontranscount_original+1)}
        #print('lines:',self.sol1['lines'])
        self.sol1['transformers'] = {
            #(r.i, r.j, r.ckt): [
            #    r.i, # iorig
            #    r.j, # idest
            ...
            #    compute_xfmr_position(r)[0]] # xst
            #for r in self.data.raw.transformers.values()}
            (acn.ourtrans[r].i, acn.ourtrans[r].j, acn.ourtrans[r].ckt): [
                acn.ourtrans[r].i, # iorig
                acn.ourtrans[r].j, # idest
                acn.ourtrans[r].ckt, # id
                tron[r-1], # x
                taps[r-1]] # xst
            for r in range(1,acn.transcount_original+1)}
        #print('transformers:',self.sol1['transformers'])
        self.sol1['switched shunts'] = {
            #self.swsh_i[i]: (
            #    [self.swsh_i[i]] + #i
            #    self.swsh_xst[i, 0:self.swsh[i].swsh_susc_count].flatten().tolist()) # xst1, xst2 ... swsh_susc_count
            #for i in range(self.num_swsh)}
            acn.ourswshunts[i].i: (
                [acn.ourswshunts[i].i] + #i
                [sw[i,a] for a in range(1,1+acn.ourswshunts[i].len) if acn.ourswshunts[i].n[a]]
                )
            for i in range(1,1 + acn.numswitchedshunts)
            if acn.ourswshunts[i].status}
        # self.sol1['switched shunts'] = {
        #     r.i: (
        #         [r.i] + # i
        #         compute_swsh_steps(r)) # xst1, xst2 ... swsh_susc_count
        #     for r in self.data.raw.switched_shunts.values()
        #     if r.stat == 1}
        #print('switched shunts',self.sol1['switched shunts'])
        #sys.exit()
        end_time = time.time()
        print('construct_sol1_new time: %f' % (end_time - start_time))
    ...
    def write_sol_base_only(self, sol_dir, fname, acn, vm, va, clearedloads, genp, genq, genon, lineon, tron, taps, sw):

        start_time = time.time()
        #print(self.data)
        self.construct_arrays()
        self.construct_sol1_new(acn, vm, va, clearedloads, genp, genq, genon, lineon, tron, taps, sw)
        filename = sol_dir + '/' + fname + '.txt'
        savename = sol_dir + '/' + fname + '.bin'
        #filename = 'solution1new.txt'
        self.write_sol_case(self.sol1, filename)
        #if saved_sol_file_name is not None:
        #    self.write_sol_bin(self.sol1, saved_sol_file_name)
        self.write_sol_bin(self.sol1, savename)
        end_time = time.time()
        print('write_sol time: %f' % (end_time - start_time))

    def write_sol_ctgs_only_new(self, sol_dir, fname, acn, vm, va):

        start_time = time.time()
        #print(self.data)
        self.construct_arrays()
        savename = sol_dir + '/' + fname + '.bin'
        self.sol1 = self.read_sol_bin(savename)
        #self.construct_sol1_bus()
        self.construct_sol2_bus_new(acn, vm, va)
        #self.construct_sol1()
        #print('buses=',self.sol1['buses'])
        #os.exit()
        #filename = sol_dir + '/' + sol_prefix + base_case_label + sol_suffix
    ...
    def write_sol_ctgs_only(self, sol_dir, fname):
