...
class Evaluation:
    ...
    #@timeit
    def eval_gen_cost(self, acn, log):
        # C2 A1 S21 #25-28
    ...
    #@timeit
    def eval_line_cost(self, acn, log):
        # C2 A1 S #17-20
    ...
    #@timeit
    def eval_bus_cost(self, acn, log):
        # C2 A1 S #3-6
        ...
        np.copyto(acn.pimb_penalty, self.bus_cost)

        log.joint('\n active power imbalance cost\n')
        maxpimb = 0

        indpimb = -1
        sumpcost = 0
        for j in range(len(self.bus_cost)):
            thisapimb = abs(self.bus_pow_real_imbalance[j])
            if thisapimb > 1e-08:
                #if thisapimb > 1e-02:
                   #log.joint(' <--- bus1 %d pimb %g cost %g\n' %(j+1, self.bus_pow_real_imbalance[j], self.bus_cost[j]))
                if thisapimb > maxpimb:
                    maxpimb = thisapimb
                    indpimb = j+1
                    log.joint(' ---> bus1 %d pimb %g cost %g\n' %(j+1, self.bus_pow_real_imbalance[j], self.bus_cost[j]))
            sumpcost += self.bus_cost[j]
        log.joint(' max p imbalance ' + str(maxpimb) + ' at bus1 ' + str(indpimb) + '\n')
        log.joint(' bus p cost (incl. delta): %.4e\n' %(self.delta*sumpcost))
        '''
        '''
        ...
        np.copyto(acn.qimb_penalty, self.bus_temp)

        log.joint('\n reactive power imbalance cost\n')
        maxqimb = 0
        indqimb = -1
        sumqcost = 0
        for j in range(len(self.bus_cost)):
            thisaqimb = abs(self.bus_pow_imag_imbalance[j])
            #if thisaqimb > 1e-02:
               #log.joint(' <--- bus1 %d qimb %g cost %g\n' %(j+1, self.bus_pow_imag_imbalance[j], self.bus_temp[j]))
            if thisaqimb > 1e-08:
                if thisaqimb > maxqimb:
                    maxqimb = thisaqimb
                    indqimb = j+1
                    log.joint(' ---> bus1 %d qimb %g cost %g\n' %(j+1, self.bus_pow_imag_imbalance[j], self.bus_temp[j]))
            sumqcost += self.bus_temp[j]
        log.joint(' max q imbalance ' + str(maxqimb) + ' at bus1 ' + str(indqimb) + '\n')
        log.joint(' bus q cost (incl. delta): %.4e\n' %(self.delta*sumqcost))


        '''
        log.joint(' reactive power imbalance cost\n')
        for j in range(len(self.bus_cost)):
            if abs(self.bus_pow_imag_imbalance[j]) > 1e-08:
                log.joint(' bus %d qimb %g cost %g\n' %(j+1, self.bus_pow_imag_imbalance[j], self.bus_cost[j]))
        '''
    ...
    #@timeit
    def eval_obj(self, acn, log):
        '''evaluate the total objective on the current case'''
        #C2 A1 S23 #2
        log.joint("-----> in eval_obj\n")

        #print(' >>>>>>>> ', self.bus_pow_real_imbalance[23], acn.pimb[23])
        ...
        log.joint(' total_bus_cost %.8e\n' %bus_cost)
        log.joint(' total_load_benefit %.8e\n' %load_benefit)
        log.joint(' total_gen_cost %.8e\n' %gen_cost)
        log.joint(' total_line_cost %.8e\n' %line_cost)
        log.joint(' total_xfmr_cost %.8e\n' %xfmr_cost)
        log.joint(' obj %.8e\n' %self.obj)

    #@timeit
    def eval_prior_bus_pow(self, acn, log):
        '''evaluate the prior point power imbalance, add to summary2'''

        self.set_sol_from_data()
        self.eval_load_pow()
        self.eval_fxsh_pow()
        self.eval_line_pow(acn, log)
        self.eval_xfmr_tap()
        self.eval_xfmr_imp_corr()
        self.eval_xfmr_adm()
        self.eval_xfmr_pow(acn, log)
        self.eval_swsh_adm()
        self.eval_swsh_pow()
        self.eval_bus_pow(acn, log)
    ...
    def eval_case(self, acn, log):
        ...
        self.eval_gen_cost(acn, log)

        # lines
        self.eval_line_xsw_bounds()
        self.eval_line_xsw_qual()
        self.eval_line_pow(acn, log)
        self.eval_line_pow_mag_max_viol(acn, log)
        self.eval_line_cost(acn, log)
        ...
        self.eval_xfmr_adm()
        self.eval_xfmr_pow(acn, log)
        self.eval_xfmr_pow_mag_max_viol(acn, log)
        self.eval_xfmr_cost()
        ...
        # bus power balance
        self.eval_bus_pow(acn, log)
        self.eval_bus_cost(acn, log)

        # total case objective
        self.eval_obj(acn, log)
    ...
    #@timeit
    def eval_line_pow(self, acn, log):
    ...
    # C2 A1 S16 #53 - #54
    #@timeit
    def eval_line_pow_mag_max_viol(self, acn,log):
    ...
    # C2 A1 S14 #68 - #71
    #@timeit
    def eval_xfmr_pow(self, acn, log):
        ...
        mykeytrdual = acn.alldata['mykeytrdual']
        log.joint('mykeytrdual: ' + str(mykeytrdual) + '\n')
        '''
        log.joint(' trans report\n')
        num = len(self.xfmr_orig_bus)
        for h in range(num):
            log.joint(' trans %d i %d j %d ckt %s orig %d dest %d\n' %(h, self.xfmr_i[h], self.xfmr_j[h], self.xfmr_ckt[h], self.xfmr_orig_bus[h], self.xfmr_dest_bus[h]))

        breakexit('reported')
        '''

        if mykeytrdual >= 0:
            mykeytr = acn.transATbuses[mykeytrdual-1].parentcount1-1

            #log.joint(' vmag_orig at ' + str(mykeytr) + ' ' + str(self.xfmr_orig_volt_mag[mykeytr]) + ' ')
            origbus = self.xfmr_orig_bus[mykeytr]
            destbus = self.xfmr_dest_bus[mykeytr]

            log.joint(' looking at (parent) trans0 ' + str(mykeytr) + '\n')
            log.joint(' orig ' + str(origbus) + ' dest ' + str(destbus) +'\n')
            log.joint(' x ' + str(self.xfmr_xsw[mykeytr]) + '\n')
            log.joint(' vmag_orig '  + str(self.xfmr_orig_volt_mag[mykeytr]) + ' ')
            log.joint(' vmag_dest '  + str(self.xfmr_dest_volt_mag[mykeytr]) + '\n')
            log.joint(' vang_orig ' + str(self.bus_volt_ang[origbus]))
            log.joint(' vang_dest ' + str(self.bus_volt_ang[destbus]) + '\n')
            log.joint(' tapangle ' + str(self.xfmr_tap_ang[mykeytr]))
            log.joint(' vang_diff ' + str(self.xfmr_volt_ang_diff[mykeytr]) + '\n')
            log.joint(' adm_real (g) ' + str(self.xfmr_adm_real[mykeytr]) + ' ')
            log.joint(' adm_imag (b) ' + str(self.xfmr_adm_imag[mykeytr]) + ' ')
            log.joint(' tap_mag ' + str(self.xfmr_tap_mag[mykeytr]) + ' ')
            log.joint(' mag_real ' + str(self.xfmr_adm_mag_real[mykeytr]) + '\n')
            #log.joint(' Gf0 ' + str(self.xfmr_adm_real[mykeytr] / self.xfmr_tap_mag[mykeytr]**2.0 + self.xfmr_adm_mag_real[mykeytr]) + '\n')
            #log.joint(' Po_orig ' + str(self.xfmr_orig_pow_real[mykeytr]) + '\n')

            log.joint('  term1 ' + str(self.xfmr_adm_real[mykeytr] * self.xfmr_dest_volt_mag_sq[mykeytr]) + '\n')
            log.joint('  term2 ' + str(- self.xfmr_adm_real[mykeytr] * self.xfmr_cos_volt_ang_diff[mykeytr]) + '\n')
            log.joint('  term3 ' + str(self.xfmr_adm_imag[mykeytr] * self.xfmr_sin_volt_ang_diff[mykeytr]) + '\n')
            log.joint(' P_orig ' + str(self.xfmr_orig_pow_real[mykeytr]) + '\n')
            log.joint(' Q_orig ' + str(self.xfmr_orig_pow_imag[mykeytr]) + '\n')
            log.joint(' P_dest ' + str(self.xfmr_dest_pow_real[mykeytr]) + '\n')
            log.joint(' Q_dest ' + str(self.xfmr_dest_pow_imag[mykeytr]) + '\n')




        '''
        for k in range(len(self.xfmr_orig_pow_real)):
            log.joint('>  GO tranformer %d [ %d , %d ] po %g pd %g\n' %(k+1, 1+self.xfmr_orig_bus[k], 1+self.xfmr_dest_bus[k], self.xfmr_orig_pow_real[k], self.xfmr_dest_pow_real[k]))
        '''
        '''
        for k in range(len(self.xfmr_orig_pow_imag)):
            log.joint('>  GO tranformer %d [ %d , %d ] qo %g qd %g\n' %(k+1, 1+self.xfmr_orig_bus[k], 1+self.xfmr_dest_bus[k], self.xfmr_orig_pow_imag[k], self.xfmr_dest_pow_imag[k]))
        '''
    ...
    # C2 A1 S16 #73 - #74
    #@timeit
    def eval_xfmr_pow_mag_max_viol(self, acn, log):
    ...
    def eval_bus_pow(self, acn, log):
        ...
        #print('  @@@@ trans ', self.bus_pow_imag_imbalance[580])
        ...
        #print('  !!!! swsh ', self.bus_swsh_matrix.dot(self.swsh_pow_imag)[580])
        #print('  @@@@ swsh ', self.bus_pow_imag_imbalance[580])

        np.copyto(acn.qimb, self.bus_pow_imag_imbalance)
        np.copyto(acn.qimb_abs, self.bus_pow_imag_imbalance)
        mykeybus = acn.alldata['mykeybus']-1
        log.joint('mykeybus0: ' + str(mykeybus) + '\n')

        if mykeybus >= 0:
            pload = self.bus_load_matrix.dot(self.load_pow_real)
            pgeneration = self.bus_gen_matrix.dot(self.gen_pow_real)
            pfxsh = self.bus_fxsh_matrix.dot(self.fxsh_pow_real)
            pnontbranch = self.bus_line_orig_matrix.dot(self.line_orig_pow_real) + self.bus_line_dest_matrix.dot(self.line_dest_pow_real)
            ptrans = self.bus_xfmr_orig_matrix.dot(self.xfmr_orig_pow_real) + self.bus_xfmr_dest_matrix.dot(self.xfmr_dest_pow_real)
            pnetnotload = pgeneration - pfxsh - pnontbranch - ptrans

            log.joint('pgeneration at bus0 %d = %.4e\n' %(mykeybus, pgeneration[mykeybus]))
            log.joint('pfxsh at bus0 %d = %.4e\n' %(mykeybus, pfxsh[mykeybus]))
            log.joint('pnontbranch at bus0 %d = %.4e\n' %(mykeybus, pnontbranch[mykeybus]))
            log.joint('ptrans at bus0 %d = %.4e\n' %(mykeybus, ptrans[mykeybus]))
            log.joint('pnetnotload at bus0 %d = %.4e\n' %(mykeybus, pnetnotload[mykeybus]))

            log.joint('pload at bus0 %d = %.4e\n' %(mykeybus, pload[mykeybus]))
            log.joint('no. of loads at bus0 %d = %d; no. of generators = %d\n' %(mykeybus, acn.ourbuses[mykeybus+1].loadcount, acn.ourbuses[mykeybus+1].gencount))


            qload = self.bus_load_matrix.dot(self.load_pow_imag)
            qgeneration = self.bus_gen_matrix.dot(self.gen_pow_imag)
            qfxsh = self.bus_fxsh_matrix.dot(self.fxsh_pow_imag)
            qnontbranch = self.bus_line_orig_matrix.dot(self.line_orig_pow_imag) + self.bus_line_dest_matrix.dot(self.line_dest_pow_imag)
            qtrans = self.bus_xfmr_orig_matrix.dot(self.xfmr_orig_pow_imag) + self.bus_xfmr_dest_matrix.dot(self.xfmr_dest_pow_imag)
            qswsh = self.bus_swsh_matrix.dot(self.swsh_pow_imag)
            qnetnotload = qgeneration - qfxsh - qnontbranch - qtrans - qswsh


            log.joint('\nqgeneration at bus0 %d = %.4e\n' %(mykeybus, qgeneration[mykeybus]))
            log.joint('qfxsh at bus0 %d = %.4e\n' %(mykeybus, qfxsh[mykeybus]))
            log.joint('qnontbranch at bus0 %d = %.4e\n' %(mykeybus, qnontbranch[mykeybus]))
            log.joint('qtrans at bus0 %d = %.4e\n' %(mykeybus, qtrans[mykeybus]))
            log.joint('qswsh at bus0 %d = %.4e\n' %(mykeybus, qswsh[mykeybus]))
            log.joint('qnetnotload at bus0 %d = %.4e\n' %(mykeybus, qnetnotload[mykeybus]))

            log.joint('qload at bus0 %d = %.4e\n' %(mykeybus, qload[mykeybus]))
            log.joint('no. of loads at bus0 %d = %d; no. of generators = %d\n' %(mykeybus, acn.ourbuses[mykeybus+1].loadcount, acn.ourbuses[mykeybus+1].gencount))

            #breakexit('just after')
...
#RAW
# evaluate just the base objective from a file
def base_from_file(acn, solution_file):

    alldata = acn.alldata
    log = alldata['log']
    raw_name = alldata['RAW']
    con_name = alldata['CON']
    sup_name = alldata['JSON']
    ...
    if not os.path.exists(f'{solution_file}'):
        solutions_exist = False
        print(solution_file)
        #breakexit('solution file not found')
        print_info(f'{solution_file} could not be found')
    ...
    '''
    arpae = alldata['arpae']  #Solver class entity
    p = arpae.data
    '''
    ...
    # and evaluate...  xxxxxxxx

    e.eval_case(acn, log)
    #e.eval_case(acn, log)

    return (e.obj,  1 if e.infeas else 0, solutions_exist, e.summary)

#RAW
# evaluate just one contingency from a file
def ctgy_from_file(acn, active_case, solution_file):

    #line_switching_allowed=None, xfmr_switching_allowed=None, check_contingencies=None):
    ...
    #### Need to do this to get base case solution values
    #### used as prior data in contingency model
    # initialize the base case solution
    sb = CaseSolution()
    sb.set_array_dims(e)
    sb.set_maps(e)
    ...
    # read the base case solution
    sb.set_read_dims()
    sb.read(f'./solution_BASECASE.txt')
    sb.set_arrays_from_dfs()
    # copy solution into evaluation
    e.set_sol(sb)
    e.round_sol()
    # and evaluate...
    # Unfortunately we need to re-evaluate the base to get the base solution
    # info (e.g. loadP0) needed for contingencies.  There should be a way to avoid
    # this so we don't waste this extra base evaluation time, every time we evaluate
    # a contingency.
    e.eval_case(acn, log)
    #e.eval_case(acn, log)
    ####
    ...
    #breakexit('read ctg')
    sc.read(f'{solution_file}')
    #breakexit('done read ctg')
    #sys.exit('bye')
...
#@timeit
def run(acn, log, raw_name, con_name, sup_name, solution_path=None, ctg_name=None, summary_name=None, detail_name=None, line_switching_allowed=None, xfmr_switching_allowed=None, check_contingencies=None):
    ...
    if process_rank == 0:
        ...
        if doit:
            ...
            log.joint(' skipping detailed contingency score report\n')

        log.joint('obj cumulative: {}\n'.format(e.summary2['obj_cumulative']))
        log.joint('infeas cumulative: {}\n'.format(e.summary2['infeas_cumulative']))
        ...
        log.joint('eval time: {}\n'.format(end_time_all - start_time_all))
