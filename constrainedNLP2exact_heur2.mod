...
# for the GO2 Competition from Team ACN.  Exact formulation with penalties
# for line current constraints.
# 04 Feb 2021
...
param genonexist{g in 1..G}; #genon from base solution: status of generator g at base solution
...
param UPntr{i in 1..NTRcnt_original}; #upper bound on binary variable ntrON used to fix it
param LOntr{i in 1..NTRcnt_original}; #lower bound on binary variable ntrON used to fix it

param UPtr{i in 1..TRcnt_original}; #upper bound on binary variable trON used to fix it
param LOtr{i in 1..TRcnt_original}; #upper bound on binary variable trON used to fix it

param UPgen{g in 1..G}; #upper bound on binary variable genon (used to fix)
param LOgen{g in 1..G}; #lower bound on binary variable genon (used to fix)
param UPgenstartup{g in 1..G}; #upper bound on binary variable genon (used to fix)
param LOgenstartup{g in 1..G}; #lower bound on binary variable genon (used to fix)
param UPgenshutdown{g in 1..G}; #upper bound on binary variable genon (used to fix)
param LOgenshutdown{g in 1..G}; #lower bound on binary variable genon (used to fix)

param UPxha{h in 1..SWSHcnt, a in 1..bhasize[h]: swshactive[h]*nha[h,a]>0};
param LOxha{h in 1..SWSHcnt, a in 1..bhasize[h]: swshactive[h]*nha[h,a]>0};
...
var xha{h in 1..SWSHcnt, a in 1..bhasize[h]: swshactive[h]*nha[h,a]>0} integer >= 0, <= nha[h,a]; #initialize 0???
...
var bhavar{h in 1..SWSHcnt: swshactive[h]>0} = sum{a in 1..bhasize[h]:nha[h,a]>0} bha[h,a]*xha[h,a];
...
var Strblock{f in 1..TRcnt_original, n in 1..Snumcblocks} >= 0, <= Smaxcblock[n]*trratingct[f];  # constraint (26)
...
# imposing hard upper bound to avoid very large penalties (may be problematic in some cases?)
...
#var xstf{f in 1..TRcnt_original} = sum{m in minxstf[f]..maxxstf[f]} m*binxstf[f,m]; #defined var

set fixedbinxstf{f in 1..TRcnt_original}; # set of m indices such that binxstf[f,m] = 0 is forced
...
var trexceed{f in 1..TRcnt_original} = sum{n in 1..Snumcblocks} Scostcblock[n]*Strblock[f,n];   #2nd part of (23) w/out delta multiplier
...
#subject to fixtotalPplus{i in 1..I}:  #avoid largest penalty
#if (Pnumcblocks>1 and Pcostcblock[Pnumcblocks]>=1e6) then Pplusblock0[i,Pnumcblocks] = 0;

subject to totalPminus{i in 1..I}: #constraint (5)
relaxPminus[i] = sum{n in 1..Pnumcblocks} Pminusblock0[i,n];

#subject to fixtotalPminus{i in 1..I}:  #avoid largest penalty
#if (Pnumcblocks>1 and Pcostcblock[Pnumcblocks]>=1e6) then Pminublock0[i,Pnumcblocks] = 0;
...
#subject to fixtotalQplus{i in 1..I}: #avoid largest penalty
#if (Qnumcblocks>1 and Qcostcblock[Qnumcblocks]>=1e6) then Qplusblock0[i,Qnumcblocks] = 0;

subject to totalQminus{i in 1..I}: #constraint (8)
relaxQminus[i] = sum{n in 1..Qnumcblocks} Qminusblock0[i,n];

#subject to fixtotalQminus{i in 1..I}: #avoid largest penalty
#if (Qnumcblocks>1 and Qcostcblock[Qnumcblocks]>=1e6) then Qminublock0[i,Qnumcblocks] = 0;
...
# Line current constraints (nontransformers); squared form (57,58)
subject to LineCurrentNTR{i in 1..NTRcnt}:
        ntrP[i]^2 + ntrQ[i]^2 - (ntrratingct[ntrp[i]]*busVmags[ntrbusi[i]])^2 <= 0;

# Line current constraints (transformers); squared form (79,80)
subject to LineCurrentTR{i in 1..TRcnt}:
	trP[i]^2 + trQ[i]^2 <= (trratingct[trp[i]] + relaxStr[trp[i]])^2;   #squared form
        #sqrt(trP[i]^2 + trQ[i]^2) - trratingct[trp[i]] <= relaxStr[trp[i]];
...
#subject to upboundxstf{f in 1..TRcnt_original}: #constraint (61) upper bound
#xstf[f] <= maxxstf[f];

#subject to loboundxstf{f in 1..TRcnt_original}: #constraint (61) lower bound
#xstf[f] >= minxstf[f];

subject to upboundxstf{f in 1..TRcnt_original: f != trout_index}: #constraint (61) upper bound
xstf[f] <= heurmaxxstf[f];

subject to loboundxstf{f in 1..TRcnt_original: f != trout_index}: #constraint (61) lower bound
xstf[f] >= heurminxstf[f];
...
sum{m in fixedbinxstf[f]} binxstf[f,m] = 0;
...
subject to upbndntrON{i in 1..NTRcnt_original: i != ntrout_index}:
ntrON[i] <= UPntr[i];

subject to lobndntrON{i in 1..NTRcnt_original: i != ntrout_index}:
ntrON[i] >= LOntr[i];

subject to upbndtrON{i in 1..TRcnt_original}:
trON[i] <= UPtr[i];

subject to lobndtrON{i in 1..TRcnt_original}:
trON[i] >= LOtr[i];


subject to upbndgenON{g in 1..G}:
genon[g] <= UPgen[g];

subject to lobndgenON{g in 1..G}:
genon[g] >= LOgen[g];

subject to upbndgenstartup{g in 1..G}:
genstartup[g] <= UPgenstartup[g];

subject to lobndgenstartup{g in 1..G}:
genstartup[g] >= LOgenstartup[g];

subject to upbndgenshutdown{g in 1..G}:
genshutdown[g] <= UPgenshutdown[g];

subject to lobndgenshutdown{g in 1..G}:
genshutdown[g] >= LOgenshutdown[g];

subject to upxha{h in 1..SWSHcnt, a in 1..bhasize[h]: swshactive[h]*nha[h,a]> 0}:
xha[h,a] <= UPxha[h,a];

subject to loxha{h in 1..SWSHcnt, a in 1..bhasize[h]: swshactive[h]*nha[h,a]> 0}:
xha[h,a] >= LOxha[h,a];
