...
param trueg{f in 1..TRcnt_original, m in -1+minxstf[f]..maxxstf[f]};
param trueb{f in 1..TRcnt_original, m in -1+minxstf[f]..maxxstf[f]};
...
var xha0{h in 1..SWSHcnt, a in 1..bhasize[h]: swshactive[h]*nha[h,a]>0} integer >= 0; #relax and round???
...
#var binxstf0{f in 1..TRcnt_original, m in minxstf[f]..maxxstf[f]} binary; #>= 0, <= 1; #used to define xstf0 in parallel, should be binary
...
- sum{i in 1..I} Delta*buscost0[i]                                                               #total bus power imbalance cost at 0
...
subject to PBalanceat0{i in 1..I}:
 sum{g in gensatbusat0[i]: gencountat0[i]>0} genP0[g]
...
subject to QBalanceat0{i in 1..I}:
 sum{g in gensatbusat0[i]: gencountat0[i]>0} genQ0[g]
 - sum{l in activeloadsatbusat0[i]: activeloadcountat0[i]>0}loadQ0[l] + fsblpu[i]*busVmags[i]^2  + (sum{s in activeswshatbus[i]: numactiveswshatbus[i]>0}bhavar0[s])*busVmags[i]^2
...
subject to LineCurrentTR{i in 1..TRcnt}:
        trP0[i]^2 + trQ0[i]^2 - trrating0[i]^2 <= 0;
...
subject to seqxstf0{f in 1..TRcnt_original, m in minxstf[f]..maxxstf[f]-1}:
...
xstf0[f] = minxstf[f] + sum{m in minxstf[f]..maxxstf[f]} binxstf0[f,m];

subject to defgf0{f in 1..TRcnt_original: Fnuvalid[f] > 0}:
  gf0[f] = sum{m in minxstf[f]..maxxstf[f]} (trueg[f,m] - trueg[f,m-1])*binxstf0[f,m];
...
subject to defbf0{f in 1..TRcnt_original: Fnuvalid[f] > 0}:
  bf0[f] = sum{m in minxstf[f]..maxxstf[f]} (trueb[f,m] - trueb[f,m-1])*binxstf0[f,m];
