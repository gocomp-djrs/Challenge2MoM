...
set truegbindices{f in 1..TRcnt_original};
...
var binxstf0{f in 1..TRcnt_original, m in minxstf[f]..maxxstf[f]} >= 0, <= 1; #used to define xstf0 in parallel, should be binary

var tauf0{f in 1..TRcnt_original}; #variable (62), tau_fk for k = 0
...
# I don't understand this constraint.   It seems like the terms related to real power should be summed over Pnumcblocks (N^p)
# but in the doc, everything is summed over Qnumcblocks (N^q)
...
subject to defGf0o{f in 1..TRcnt: tr_o_or_d[f] == 1}:
  Gf0[f] = gf0[trp[f]]/(tauf0[trp[f]]^2) +  gmf[trp[f]];

subject to defGf0d{f in 1..TRcnt: tr_o_or_d[f] == 0}:
  Gf0[f] = gf0[trp[f]];

subject to defBf0o{f in 1..TRcnt: tr_o_or_d[f] == 1}:
  Bf0[f] = bf0[trp[f]]/(tauf0[trp[f]]^2) +  bmf[trp[f]];

subject to defBf0d{f in 1..TRcnt: tr_o_or_d[f] == 0}:
  Bf0[f] = bf0[trp[f]];
